...
* We invested <b>`r sum(Hours$Duration[Hours$isTraining]) %>% round() %>% comma()` 
hours training</b> volunteers
...
ggplot(aes(x=Animal, y=Type, label=round(nVol))) + 
  ...
  labs(x="Animal Type", y="Activity Type")
...
Hours %>%
  ...
  coord_polar(theta = "y") + 
  theme_UT() + labs(x="", y="", title="Percentage of Total Hours, By Activity Type")
```
...
Hours %>%
  ...
  coord_polar(theta = "y") + 
  theme_UT() + labs(x="", y="", title="Percentage of Total Hours, By Animal Type")
```
...
Hours %>%
  ...
  guides(fill=guide_legend(nrow=2,byrow=TRUE)) +
  coord_polar(theta = "y") + 
  theme_UT() + labs(x="", y="", title="Percentage of Total Hours, By Location")
```
...
Hours %>%
  ...
  ggplot(aes(x=Type, y=HrPerVol, fill=Animal, alpha=sum.Hours/1000)) + 
  geom_bar(width = 1, stat="identity", color="black") + 
  scale_alpha_continuous(name="Total Hours For Activity\n(x1000)", range=c(0.5, 1)) +
  theme_UT() + coord_polar() + 
  labs(x="", y="Average Hours Per Volunteer", title="Activity Type Hours, By Animal Type")
```
...
Hours %>%
  ...
  scale_fill_brewer(palette = "Dark2") + guides(fill=guide_legend(nrow=2,byrow=TRUE)) +
  coord_flip() + theme_UT() + labs(x="Type of Activity", y="Total Hours (x1000)",
                    title="Total Hours By Activity Type, By Animal")
...
```{r Hours Fig06}
Hours %>%
  group_by(User.ID, Type, Animal, Location) %>%
  summarise(
    Hours    = sum(Duration),
    nEntries = n()
  ) %>% ungroup() %>%
  filter(#Hours>median(Hours), 
         Location!="Events",
         Location!="Offsite"
         ) %>%
  mutate(Rank = min_rank(desc(Hours))) %>%
  # summary()

  # Plot
  ggplot(aes(x=Rank, fill=Type)) + 
  facet_grid(Animal~Location, scales = "free") +
  geom_density(alpha=0.5) +
    ## scales
  scale_fill_brewer(palette = "Dark2") + guides(fill=guide_legend(nrow=2,byrow=TRUE)) +
  scale_x_sqrt() +
    ## theme
  # facet_grid(Location~Animal, scales = "free") +
  theme_UT() + labs(x="Time") 
